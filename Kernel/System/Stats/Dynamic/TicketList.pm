...
sub GetStatTable {
    ...
    for my $TicketID (@TicketIDs) {
		...
		my %Article = $ArticleBackendObject->ArticleGet(
			...
			ArticleID     => $Articles[0]->{ArticleID},   # (required)
		);

		my $ArticleTextString = $Kernel::OM->Get('Kernel::Output::HTML::Layout')->RichText2Ascii(
			String => $Article{Body},
		);
		
		$Ticket{FirstArticle} = $ArticleTextString;
